...
function SectionObservacoesInner({ data, loaded, onUpdate }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 lg:items-start lg:gap-6">
      <div className="rounded-2xl bg-card p-5 lg:col-span-1 lg:p-6">
        ...
        <div className="flex flex-col gap-3">
          {data.observations.slice(0, Math.ceil(data.observations.length / 2)).map((value, idx) => (
            <div key={`obs-${idx + 1}`} className="flex items-start gap-3">
              ...
              <Textarea
                className="min-h-[42px] flex-1 resize-none rounded-xl border-transparent bg-muted/60 text-sm transition-colors placeholder:text-muted-foreground/50 focus:border-primary/30 focus:bg-card focus:ring-2 focus:ring-primary/15 lg:min-h-[56px] lg:text-base"
      ...
      <div className="flex flex-col gap-4 lg:gap-6">
        ...
        {/* Motivational card */}
        <div className="rounded-2xl bg-primary p-5 text-primary-foreground lg:p-6">
          <p className="text-base font-bold lg:text-lg">Maternidade e amor</p>
          <p className="mt-1.5 text-sm leading-relaxed text-primary-foreground/75 lg:text-base">
          ...
          <p className="mt-3 font-serif text-xl lg:text-2xl">Seja bem-vinda!</p>
